...
def add_regions(ax, chromsizes, regions=None, region_names=None, sep:Optional='\t'):
    ...
    if regions is None:
        ...
        assert isinstance(region_names, list), "region_names should be a list."
        assert len(regions) == len(region_names), "Regions and region_names should be the same length."
        for i, r in enumerate(regions):
            color = f'C{i}'

            # add legend element per regions
            leg_lab.append(f"{region_names[i]}")
            leg.append(Line2D([0], [0], color=color, lw=4))
